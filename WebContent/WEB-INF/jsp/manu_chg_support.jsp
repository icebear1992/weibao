...
<link href='<c:url value="/date/jquery-ui-timepicker-addon.min.css"/>' rel="stylesheet" />
	...
	<body onload="getregion();judge()">
		...
		<div class="alert alert-warning"><h3 align="center"><span class="glyphicon glyphicon-phone-alt"></span> 服务响应修改</h3></div>
		<br />
		<form method="post" class="form-horizontal" role="form" id="form" action='<c:url value="/provider/ChgSER.do"/>'>
			<div class="container">
				<div class="col-lg-1">
			...
			<div align="left" style="font-size: 16px">
				开始日期:<input type="text" id="defult1" name="startTime" value="${input.ser.startTime}" readonly="readonly"> <br /><br />
				结束日期:<input type="text" id="defult2" name="endTime" value="${input.ser.endTime}" readonly="readonly"><br /><br />
				...
				区&nbsp;&nbsp;&nbsp;&nbsp;域:&nbsp;&nbsp;&nbsp;<select class="combsobox" name="region" onchange="getSubRegions()" id="region">
				    ...
				    <select class="combobox" name="subRegion" id="subregion" >
						<option value="${input.ser.subRegion.subRegionId}"><c:out value="${input.ser.subRegion.subRegionName}" /></option>
					</select><br /><br />
					专&nbsp;&nbsp;&nbsp;&nbsp;业:&nbsp;&nbsp;
					<select class="combobox" name="major" id="major" onchange="getMajorContact()" onblur="getNetWork()">
						<option value="${input.ser.neModel.getNe().network.major.majorId}"><c:out value="${input.ser.neModel.getNe().network.major.majorName}" /></option>
					...
					<option value="${input.ser.auditor.tpId}"><c:out value="${input.ser.auditor.tpName}" /></option>
					</select>
	            <br /><br />
				联系方式:<select class="combobox" name="contactinfo" id="contactinfo" >
						<option value="${input.ser.contactinfo }"><c:out value="${input.ser.contactinfo}" /></option>
				...
				网&nbsp;&nbsp;&nbsp;&nbsp;络:&nbsp;&nbsp;&nbsp;<select name="network" id="network" onchange="getNE()">
							...
							<option value="${input.ser.neModel.neModelId}"><c:out value="${input.ser.neModel.neModelName}" /></option>
							</select><br /><br />
				</div>
			</div>
		</div>
		<div class="col-lg-5">
			<div align="left" style="font-size: 16px">
				支&nbsp;撑&nbsp;方&nbsp;式:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<select class="combobox" name="supportMethod" id="supportmethod" >
						<option value="${input.ser.supportMethod}"><c:out value="${input.ser.supportMethod}" /></option>
						<option value="远程">远程</option>
						<option value="现场">现场</option>
						<option value="远程+现场">远程+现场</option>
					</select><br /><br />
				主要处理人:&nbsp;&nbsp;&nbsp;&nbsp;<input name="majorHandler" id="majorHandler" value="${input.ser.majorHandler}" /> <br /><br />
				远程服务时长:&nbsp;<input name="telnetservDuration" id="telnetservDuration" value="${input.ser.telnetServDuration}" /><br /><br />
				现场服务时长:&nbsp;<input name="fieldservDuration" id="fieldservDuration" value="${input.ser.fieldServDuration}" /><br /><br />
				问题及需求类别:&nbsp;&nbsp;<select class="combobox" name="demand" id="demand" >
						<option value="${input.ser.category.demandId}" ><c:out value="${input.ser.category.demandName}" /></option>
				...
				问题及需求描述:
				<textarea class="form-control" rows="1" id="demandinfo" name="demandinfo">
				<c:out value="${input.ser.demandInfo}" />
				</textarea>
				解决方案:
				<textarea class="form-control" rows="1" id="solution" name="solution">
				<c:out value="${input.ser.solution}" />
				</textarea>
				备注信息:
				<textarea class="form-control" rows="1" id="memoinfo" name="memoinfo">
				<c:out value="${input.ser.remarksInfo}" />
				</textarea>
				</div>
		...
		<div class="col-lg-1">
			...
			<div class="form-group" align="center">
			    <input type="hidden" name="serId" value="${input.ser.serId}">
