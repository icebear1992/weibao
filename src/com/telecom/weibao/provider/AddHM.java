...
public class AddHM extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try{
			...
			java.util.Date completionTimeUtil =sdf.parse("2010-01-01");
			if (request.getParameter("end") != null) {
				try {
					completionTimeUtil = sdf.parse(request.getParameter("completionTime"));
				} catch (Exception e) {
				}
			}
			...
			for (int i = 0; i < totalNums.length; i++) {
				...
				if (totalNum>0 && repairNum >= 0 && totalNum >= repairNum) {
