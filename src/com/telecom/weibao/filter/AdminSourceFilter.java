...
@WebFilter("/AdminSourceFilter")
public class AdminSourceFilter implements Filter {
	private static Logger logger = Logger.getLogger(AdminSourceFilter.class);

	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		System.out.println("AdminSourceFilter被使用");
		...
		try {
			...
			if (userType < 0) {
				...
				logger.warn("厂商用户:" + mp.getMpName() + ":" + mp.getMpPhone() + "尝试访问超级用户资源。");
				...
				if (role > 2) {
					logger.info("超级用户:" + tp.getTpName() + ":" + tp.getTpPhone() + "正常访问超级用户资源。");
					chain.doFilter(request, response);
				} else {
					logger.warn("电信用户:" + tp.getTpName() + ":" + tp.getTpPhone() + "尝试访问超级用户资源。");
					PrintWriter out = httpServletResponse.getWriter();
					out.print("<script language='javascript'>alert('您无权对该资源进行访问！');window.history.back();</script>");
					out.close();
				}
