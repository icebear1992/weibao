...
@WebServlet("/man/GetMonthRegion")
public class GetMonthRegion extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			List<Map<String, Object>> monRegInfoList = getMonthRegionInfoList(request);
			...
			JSONArray jsonarr = JSONArray.fromObject(monRegInfoList, jsonConfig);
			// new
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String excelName = "月度报表(按区域)";
		// 转码防止乱码
		response.addHeader("Content-Disposition",
				"attachment;filename=" + new String(excelName.getBytes("gb2312"), "ISO8859-1") + ".xls");
		String[] headers = new String[] { "月份", "区域", "驻点服务(人*日)", "巡检覆盖设备数", "巡检覆盖业务节点数", "培训时长(人*时)", "方案咨询次数",
				"完结故障处理数", "未完结故障处理数", "硬件返修批次(新增)", "硬件返修批次(历史)", "硬件返修平均历时", "紧急备件服务次数" };
		String[] keys = new String[] { "time", "region_name", "serpersonnel", "totalequipment", "totalenginerroom",
				"trainingduration", "consultation", "completion", "incompletion", "ssum", "insuum", "avgtime",
				"sprcount" };
		try {
			OutputStream out = response.getOutputStream();
			List<Map<String, Object>> monRegInfoList = getMonthRegionInfoList(request);
			FileExport.exportExcel(excelName, headers, monRegInfoList, keys, out, "yyyy-MM-dd");
			out.close();
	...
	private List<Map<String, Object>> getMonthRegionInfoList(HttpServletRequest request) {
		...
		return MainStatisticsDac.getMonthRegionInfos(beginTime, endTime, region);
	}
}
